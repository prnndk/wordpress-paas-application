...
export const InstanceDetails: React.FC = () => {
	...
	// Ref to store previous instance data for comparison
	const prevInstanceRef = useRef<any>(null);

	// Helper function to check if instance data has meaningfully changed
	const hasInstanceChanged = useCallback((prev: any, next: any): boolean => {
		...
		return (
			...
			prev.runningReplicas !== next.runningReplicas ||
			prev.name !== next.name ||
			JSON.stringify(prev.endpoints) !== JSON.stringify(next.endpoints) ||
			JSON.stringify(prev.specs) !== JSON.stringify(next.specs) ||
			JSON.stringify(prev.db) !== JSON.stringify(next.db) ||
			JSON.stringify(prev.internal) !== JSON.stringify(next.internal)
		);
	}, []);
	...
	const fetchInstanceDetails = useCallback(
		async (silent = false) => {
			...
			try {
				...
				// Only update instance state if data actually changed
				if (hasInstanceChanged(prevInstanceRef.current, data)) {
					setInstance(data);
					prevInstanceRef.current = data;
				}
			} catch (error: any) {
		...
		[id, hasInstanceChanged]
	);

	// Initial fetch on mount and when ID changes
	useEffect(() => {
		if (!id) return;
		prevInstanceRef.current = null; // Reset previous instance
		fetchInstanceDetails(false); // Initial load with loading spinner
	}, [id, fetchInstanceDetails]);
		...
		<div className='space-y-6 animate-in fade-in duration-300 relative'>
			...
			<div className='bg-white shadow rounded-xl border border-slate-200 overflow-hidden'>
				...
				<div className='p-6 bg-slate-50 min-h-[600px]'>
					...
					{activeTab === "analytics" && (
						<AnalyticsTab instanceId={instance.id} />
					)}
					...
					{activeTab === "admin" && isAdmin && (
						<AdminTab
							...
							onOpenInspect={() => setModalType("inspect")}
						/>
					)}
				</div>
			</div>
