...
export const AnalyticsTab: React.FC<AnalyticsTabProps> = ({ instanceId }) => {
	...
	const fetchMetrics = useCallback(async () => {
		...
		try {
			...
			const metricsResponse = await dashboardService.getPrometheusMetrics(
				instanceId
			);
			...
			try {
				const historyResponse = await dashboardService.getPrometheusHistory(
					instanceId,
					timeRange
				);
	...
	const totalNetwork =
		(metrics?.network?.rxBytes || 0) + (metrics?.network?.txBytes || 0);

	// Storage metrics
	const storageUsed = metrics?.storage?.used || 0;
	const storageTotal = metrics?.storage?.total || 1;
				...
				<div className='bg-white p-6 rounded-xl border border-slate-200 shadow-sm'>
					...
					<div className='text-center mt-4 pt-4 border-t border-slate-100'>
						...
						<span className='text-xs text-slate-500 ml-2'>
							/ {(memoryLimit / (1024 * 1024 * 1024)).toFixed(1)} GB (
				...
				<div className='bg-white p-6 rounded-xl border border-slate-200 shadow-sm'>
					...
					<div className='relative pt-1'>
						...
						<div className='flex justify-between text-sm text-slate-600 font-medium'>
							...
							<span>
								{(storageTotal / (1024 * 1024 * 1024)).toFixed(1)} GB total
							</span>
						</div>
					</div>
