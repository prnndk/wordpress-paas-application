...
export const AdminServiceDetailPage: React.FC = () => {
    ...
    const openResourceModal = () => {
        ...
        const cpuLimit = service?.resources?.cpuLimit
            ? (service.resources.cpuLimit / 1e9).toFixed(1)
            : "";
        const memoryLimit = service?.resources?.memoryLimit
            ? Math.round(service.resources.memoryLimit / (1024 * 1024)).toString()
            : "";
        const cpuReservation = service?.resources?.cpuReservation
            ? (service.resources.cpuReservation / 1e9).toFixed(1)
            : "";
        const memoryReservation = service?.resources?.memoryReservation
            ? Math.round(service.resources.memoryReservation / (1024 * 1024)).toString()
            : "";

        setResourceForm({ cpuLimit, memoryLimit, cpuReservation, memoryReservation });
    ...
    const saveResources = async () => {
        ...
        try {
            ...
            if (resourceForm.cpuLimit) {
                resources.cpuLimit = parseFloat(resourceForm.cpuLimit);
            }
            if (resourceForm.memoryLimit) {
                resources.memoryLimit = parseInt(resourceForm.memoryLimit, 10);
            }
            if (resourceForm.cpuReservation) {
                resources.cpuReservation = parseFloat(resourceForm.cpuReservation);
            }
            if (resourceForm.memoryReservation) {
                resources.memoryReservation = parseInt(resourceForm.memoryReservation, 10);
            }

            if (Object.keys(resources).length === 0) {
                setResourceError("Please provide at least one resource value to update");
                return;
            }

            await adminService.updateServiceResources(service.name, resources);
            setResourceSuccess(true);

            // Refresh service data after successful update
            setTimeout(() => {
                fetchService();
            ...
            setResourceError(err.message || "Failed to update resources");
        } finally {
        ...
        <div className="max-w-5xl mx-auto pb-12">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 mb-6 text-white">
                ...
                <div className="grid grid-cols-4 gap-4 mt-6">
                    <div
                        onClick={openResourceModal}
                        className="bg-white/5 rounded-xl p-3 cursor-pointer hover:bg-white/10 transition-colors group">
                        <div className="flex items-center justify-between text-slate-400 text-xs mb-1">
                            <div className="flex items-center gap-2">
                                <Cpu className="w-3.5 h-3.5" /> CPU Limit
                            </div>
            ...
            {showResourceModal && service && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowResourceModal(false)}>
                    <div
                        className="bg-white rounded-2xl shadow-2xl w-full max-w-lg"
                        onClick={(e) => e.stopPropagation()}>
                        {/* Modal Header */}
                        <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-indigo-600 to-violet-600 rounded-t-2xl text-white">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h2 className="text-lg font-semibold">Edit Resource Limits</h2>
                                    <p className="text-sm text-indigo-100">{service.name}</p>
                                </div>
                                <button
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            {resourceSuccess && (
                                <div className="p-3 bg-green-50 text-green-700 rounded-lg text-sm border border-green-200 flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setResourceForm({ ...resourceForm, cpuLimit: e.target.value })}
                                            placeholder="e.g., 0.5"
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                        />
                                        <p className="text-xs text-slate-500 mt-1">Max CPU cores allowed</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">
                                            Memory Limit (MB)
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setResourceForm({ ...resourceForm, memoryLimit: e.target.value })}
                                            placeholder="e.g., 512"
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                        />
                                        <p className="text-xs text-slate-500 mt-1">Max memory in megabytes</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            step="0.1"
                                            min="0.1"
                                            value={resourceForm.cpuReservation}
                                            onChange={(e) => setResourceForm({ ...resourceForm, cpuReservation: e.target.value })}
                                            placeholder="e.g., 0.25"
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                        />
                                        <p className="text-xs text-slate-500 mt-1">Guaranteed CPU cores</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">
                                        ...
                                        <input
                                            type="number"
                                            min="64"
                                            value={resourceForm.memoryReservation}
                                            onChange={(e) => setResourceForm({ ...resourceForm, memoryReservation: e.target.value })}
                                            placeholder="e.g., 256"
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                        />
                                        <p className="text-xs text-slate-500 mt-1">Guaranteed memory in MB</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-3 bg-blue-50 text-blue-800 rounded-lg text-xs border border-blue-200">
                                <strong>Note:</strong> Updating resource limits will trigger a service restart.
