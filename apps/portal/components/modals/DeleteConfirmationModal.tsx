...
> = ({ isOpen, onClose, onConfirm, instanceName }) => {
	...
	const handleConfirm = () => {
		if (inputName.toLowerCase() !== instanceName.toLowerCase()) return;
		setIsDeleting(true);
			...
			<div className='relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200'>
				<div className='p-6 text-center'>
					...
					<p className='text-slate-600 text-sm mb-6'>
						...
						<strong className='text-slate-900'> {instanceName}</strong>{" "}
						instance, its database, and all associated backups.
					</p>

					<div className='text-left mb-6'>
						<label className='block text-xs font-bold text-slate-500 uppercase mb-2'>
							Type{" "}
							<span className='select-all text-slate-900'>{instanceName}</span>{" "}
							to confirm
						</label>
						<input
							type='text'
					...
					<div className='flex gap-3'>
						...
						<button
							...
							disabled={
								inputName.toLowerCase() !== instanceName.toLowerCase() ||
								isDeleting
							}
