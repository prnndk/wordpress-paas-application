...
export class TenantsController {
    ...
    @ApiResponse({
        status: 403,
        description: "Not authorized to view this instance",
    })
    ...
    async get(
        ...
        // Role-based access: Users can only view their own instances, admins can view all
        const isAdmin = req.user.roles?.includes("admin");
        if (!isAdmin && tenant.userId !== req.user.id) {
            throw new ForbiddenException("Not authorized to view this instance");
        }
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({
        summary: "Scale the number of replicas for a WordPress instance",
    })
    @ApiParam({ name: "id", description: "Tenant ID" })
    @ApiResponse({ status: 200, description: "Instance scaled successfully" })
    @ApiResponse({ status: 400, description: "Invalid replicas count" })
    @ApiResponse({ status: 404, description: "Instance not found" })
    async scale(
        @Param("id") id: string,
        ...
        return this.tenantsService.scaleReplicas(id, body.replicas);
    }
    ...
    @Get(":id/inspect")
    @ApiOperation({
        summary: "Get detailed container inspection",
        description: "Returns detailed information about the container including environment variables (sensitive values masked), mounts, networks, resources, and task status."
    })
    @ApiParam({ name: "id", description: "Tenant ID" })
    @ApiResponse({ status: 200, description: "Container inspection details" })
    @ApiResponse({ status: 404, description: "Instance not found" })
    async getContainerInspect(@Param("id") id: string): Promise<any> {
        const tenant = await this.tenantsService.getTenant(id);
        if (!tenant) {
            throw new NotFoundException("Instance not found");
        }
        const inspect = await this.tenantsService.getContainerInspect(id);
        if (!inspect) {
