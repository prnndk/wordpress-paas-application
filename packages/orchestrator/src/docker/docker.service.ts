...
export class DockerService implements OnModuleInit {
    ...
    async listAllServicesWithTasks(): Promise<{
        ...
        const nodeMap = new Map<string, { hostname: string; status: string; role: string; availability: string; address: string }>();
        ...
        for (const svc of services) {
            ...
            const runningTasks = taskInfos.filter(t => t.state === 'running').length;
